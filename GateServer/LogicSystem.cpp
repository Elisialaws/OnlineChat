...
LogicSystem::LogicSystem() {
	...
	RegPost("/reset_pwd", [](std::shared_ptr<HttpConnection> connection) {
		auto body_str = boost::beast::buffers_to_string(connection->_request.body().data());//返回一个Buffer
		std::cout << "receive body is " << body_str << std::endl;
		connection->_response.set(http::field::content_type, "text/json");
		Json::Value root;//给对方返回的
		Json::Reader reader;
		Json::Value src_root;
		bool parse_success = reader.parse(body_str, src_root);
		if (!parse_success) {
		...
		auto email = src_root["email"].asString();//fetch source data
		auto name = src_root["user"].asString();//json value->string
		auto pwd = src_root["passwd"].asString();
		...
		root["email"] = email;
		root["user"] = name;
		root["passwd"] = pwd; //返回root序列化后的string
		root["varifycode"] = src_root["varifycode"].asString();
		std::string jsonstr = root.toStyledString();
		beast::ostream(connection->_response.body()) << jsonstr;
		return true;
...
{
	_get_handlers.insert({ url,handler }); //向数据结构里插入
}

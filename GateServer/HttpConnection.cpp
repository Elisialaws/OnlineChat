...
{
	auto self = shared_from_this();
	_response.content_length(_response.body().size());//设置长度，粘包处理
	http::async_write(_socket,_response,[self](beast::error_code ec,std::size_t bytes_transferred) {
		self->_socket.shutdown(tcp::socket::shutdown_send, ec);//关闭服务器端
		self->_deadline.cancel();//收到回调，取消定时器计时
		});
}
...
unsigned char ToHex(unsigned char x) //10转16进制
{
...
std::string UrlDecode(const std::string& str)
{
	...
	for (size_t i = 0; i < length; i++)
	{
		...
		else if (str[i] == '%')
		{
			...
			unsigned char low = FromHex((unsigned char)str[++i]);//低四位转为16进制
			strTemp += high * 16 + low;
		}
		else strTemp += str[i];
	}
...
void HttpConnection::PreParseGetParam()
{
	...
	if (query_pos == std::string::npos) { //没有?,即没有参数
		_get_url = uri;
		return;
	}

...
#include"global.h"
#include"httpmgr.h"
//在.cpp 文件中包含需要的头文件（而非在其他头文件之间相互包含）可以帮助避免互引用的问题。
    ...
    ui(new Ui::RegisterDialog),_countdown(5)
{
    ...
    connect(HttpMgr::GetInstance().get(),&HttpMgr::sig_reg_mod_finish,
            this,&RegisterDialog::slot_reg_mod_finish);//注册界面连接sig_reg_mod_finish信号
    initHttpHandlers();
    ui->err_tip->clear();
...
void RegisterDialog::on_get_code_clicked()
{
    ...
    if(match){
        //send verify code, a post request
...
void RegisterDialog::initHttpHandlers()
{
    //注册获取验证码回包逻辑
      _handlers.insert(ReqId::ID_GET_VARIFY_CODE, [this](QJsonObject jsonObj){
          int error = jsonObj["error"].toInt();//json对象转为Int
...
void RegisterDialog::on_sure_btn_clicked()
{
    ...
    bool RegisterDialog::checkPassValid()
    {
        ...
        if(pass.length()<6||pass.length()>15){
            AddTipErr(TipErr::TIP_PWD_ERR,tr("密码长度为6~15"));
            return false;
        }
...
{
    _countdown_timer->stop();
      emit sigSwitchLogin();//取消登录发送切换登陆信号
}

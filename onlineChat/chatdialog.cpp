...
ChatDialog::ChatDialog(QWidget *parent) :
    QDialog(parent),_b_loading(false),
    ui(new Ui::ChatDialog),_mode(ChatUIMode::ChatMode),_state(ChatUIMode::ChatMode)
{
    ...
    //设置清除动作
    ui->search_edit->addAction(searchAction,QLineEdit::LeadingPosition);//action,position
      ui->search_edit->setPlaceholderText(QStringLiteral("搜索"));
      QAction *clearAction=new QAction(ui->search_edit);
      clearAction->setIcon(QIcon(":/res/close_transparent.png"));
         // 初始时不显示清除图标
         // 将清除动作添加到LineEdit的末尾位置
      ui->search_edit->addAction(clearAction, QLineEdit::TrailingPosition);//添加到末尾位置
      ...
      // 连接清除动作的触发信号到槽函数，用于清除文本
         ...
         QPixmap pixmap(":/res/head_1.jpg");
           ...
           ui->side_chat_lb->setProperty("state","normal");//side-bar聊天和联系人的属性,为stateWdiget
          ...
          connect(ui->side_chat_lb,&StateWidget::clicked,this,&ChatDialog::slot_side_chat);//切换聊天页面和联系人页面
          connect(ui->side_contact_lb, &StateWidget::clicked, this, &ChatDialog::slot_side_contact);
          connect(ui->search_edit,&QLineEdit::textChanged,this,&ChatDialog::slot_text_changed);//输入框搜索变化,显示搜索页面
          this->installEventFilter(this);//检测鼠标点击位置判断是否清空搜索框
          ui->side_chat_lb->SetSelected(true);
          SetSelectChatItem();//默认选中条目为0
          SetSelectChatPage();//更新聊天界面信息,默认聊天页面被选中
          //连接加载联系人的信号和槽函数
...
{
    if(_mode!=ChatUIMode::SearchMode){ //在搜索模式下才处理
        return;
    }
...
void ChatDialog::loadMoreChatUser()
{
    ...
    if(!friend_list.empty()){
        for(auto & friend_ele : friend_list){
             ...
             auto *chat_user_wid = new ChatUserWid();//创建新的ChatUserWidget
             auto user_info = std::make_shared<UserInfo>(friend_ele);
             chat_user_wid->SetInfo(user_info);
             QListWidgetItem *item = new QListWidgetItem;
             //qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
...
void ChatDialog::slot_side_chat()
{
    ...
    ui->stackedWidget->setCurrentWidget(ui->chat_page);//显示聊天页面
...
void ChatDialog::slot_side_contact()
{
    ...
    ui->stackedWidget->setCurrentWidget(ui->friend_apply_page);//切换显示页面，用stackWidget管理
...
void ChatDialog::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    qDebug() << "receive slot_auth_rsp uid is " << auth_rsp->_uid
       ...
       UserMgr::GetInstance()->AddFriend(auth_rsp);//添加好友，放到map里建立映射
       int randomValue = QRandomGenerator::global()->bounded(100); // 生成0到99之间的随机整数
       int str_i = randomValue % strs.size();
       int head_i = randomValue % heads.size();
       int name_i = randomValue % names.size();
       auto *chat_user_wid=new ChatUserWid();//创建新的好友widget
       auto user_info=std::make_shared<UserInfo>(auth_rsp);
       chat_user_wid->SetInfo(user_info);
       QListWidgetItem *item=new QListWidgetItem();
       item->setSizeHint(chat_user_wid->sizeHint());
       ui->chat_user_list->insertItem(0,item);//插入到最新一行
       ui->chat_user_list->setItemWidget(item, chat_user_wid);
...
void ChatDialog::slot_jump_chat_item(std::shared_ptr<SearchInfo> si)
{
    qDebug() << "slot jump chat item " << endl;
      ...
      if(find_iter!=_chat_items_added.end()){
         ...
         ui->side_chat_lb->SetSelected(true);//设置选中状态
...
void ChatDialog::slot_item_clicked(QListWidgetItem *item)
{
    ...
    if(itemType == ListItemType::INVALID_ITEM
        ...
        qDebug()<< "slot invalid item clicked ";//不切换
        return;
    }
...
void ChatDialog::slot_text_chat_msg(std::shared_ptr<TextChatMsg> msg)
{
    ...
    if(find_iter!=_chat_items_added.end()){
        ...
        if(!chat_wid)return;
        chat_wid->updateLastMsg(msg->_chat_msgs);//更新消息
        return;
    }
    //如果没找到，则创建新的插入listwidget
...
void ChatDialog::slot_append_send_chat_msg(std::shared_ptr<TextChatData> msgdata)
{
    ...
    auto itemType = customItem->GetItemType();
    if (itemType == CHAT_USER_ITEM) {
        auto con_item = qobject_cast<ChatUserWid*>(customItem);//取出chatUuserWidget拼到user_info的msg列表里，保存历史消息记录
        if (!con_item) {
            return;
        }

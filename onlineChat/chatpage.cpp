...
void ChatPage::SetUserInfo(std::shared_ptr<UserInfo> user_info)
{
    ...
    ui->chat_data_list->removeAllItem();//移除和其他用户的对话
...
/**
 * @brief ChatPage::paintEvent
 * @param event
 * 实现样式更新，需要重写paintEvent
 */
void ChatPage::paintEvent(QPaintEvent *event)
{
    QStyleOption opt;
    opt.init(this);
    QPainter p(this);//绑定窗口
    style()->drawPrimitive(QStyle::PE_Widget,&opt,&p,this);
}
/**
 * @brief ChatPage::on_send_btn_clicked
 * send message to chatPage
 */
void ChatPage::on_send_btn_clicked()
{
    ...
    auto pTextEdit = ui->chatEdit;
      ...
      const QVector<MsgInfo>& msgList = pTextEdit->getMsgList();//假定发送的为消息列表，实际为单条消息
      QJsonObject textObj;
      ...
      for(int i=0; i<msgList.size(); ++i)
      {
          ...
          ChatItemBase *pChatItem = new ChatItemBase(role);//构造一条聊天信息的item
          ...
          {
              //生成唯一id
              QUuid uuid = QUuid::createUuid();//用来标识唯一一条消息，可以用来处理是否已读等逻辑
              //转为字符串
              ...
              if(txt_size + msgList[i].content.length()> 1024){
                  ...
                  QByteArray jsonData = doc.toJson(QJsonDocument::Compact);//压缩数据
                  //发送并清空之前累计的文本列表
                  txt_size = 0;
...
void ChatPage::on_receive_btn_clicked()
{
    ...
    for(int i=0;i<msgList.size();++i){
        QString type=msgList[i].msgFlag;//先判断类型
        ChatItemBase *pChatItem=new ChatItemBase(role);
        pChatItem->setUserName(userName);

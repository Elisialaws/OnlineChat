...
{
    mGetMsgList.clear();//情空之前的消息列表

    QString doc = this->document()->toPlainText();//获取当前文档所有文本内容
    QString text="";//暂时存储文本信息
    int indexUrl = 0;
    int count = mMsgList.size();

    for(int index=0; index<doc.size(); index++)//遍历文档所有文本
    {
        if(doc[index]==QChar::ObjectReplacementCharacter)//是对象替换字符，表示一段文本的结束或者分割
        {
            if(!text.isEmpty())
            {
                QPixmap pix;
            ...
            while(indexUrl<count)
            {
                ...
                //检查文档的 HTML 表示中是否包含当前消息的内容（区分大小写）。
        ...
        {
            text.append(doc[index]);//如果当前字符不是对象替换字符，则将其附加到字符串 text 中。
        }
    }
    if(!text.isEmpty())//剩余的文本内容添加进去
    {
        QPixmap pix;
        insertMsgList(mGetMsgList,"text",text,pix);
        text.clear();

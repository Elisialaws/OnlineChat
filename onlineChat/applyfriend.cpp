...
/**
 * @brief ApplyFriend::ApplyFriend
 * @param parent
 * 在Qt中，显示一个对话框一般有两种方式，一种是使用exec()方法，它总是以模态来显示对话框；
 * 另一种是使用show()方法，它使得对话框既可以模态显示，也可以非模态显示，决定它是模态还是非模态的是对话框的modal属性。
 * 模态对话框在没有被关闭之前，用户不能与同一个应用程序的其他窗口进行交互，直到该对话框关闭。
 */
    ...
    ui(new Ui::ApplyFriend)
{
    ...
    ui->name_ed->setPlaceholderText(tr("DreamWeaver"));
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
     ui->back_ed->setPlaceholderText("DragonFlame");
     ...
     ui->scrollArea->verticalScrollBar()->setHidden(true);//取消滚动条
...
{
    lb->move(cur_point);//移动位置
    lb->show();
    _add_labels.insert(lb->text(),lb);
    _add_label_keys.push_back(lb->text());
    next_point.setX(lb->pos().x() + text_width + 15);//添加label时坐标要偏移
    next_point.setY(lb->pos().y());
}
/**
 * @brief ApplyFriend::eventFilter
 * @param obj
 * @param event
 * @return
 * 当鼠标滑入时才显示滚动条
...
void ApplyFriend::SlotLabelEnter(){
    ...
    addLabel(ui->lb_ed->text());
       ...
       //找到了就只需设置状态为选中即可
       if (find_it == _tip_data.end()) {//没找到就添加
...
/**
 * @brief ApplyFriend::SlotLabelTextChange
 * @param text
 * 标签文本变化
 */
...
void ApplyFriend::SlotApplySure()
{
    ...
    if(name.isEmpty()){
        name = ui->name_ed->placeholderText();//名称为空则为设置为占位符
    }
    ...
    QByteArray jsonData = doc.toJson(QJsonDocument::Compact);//序列化为字节流,压缩方式

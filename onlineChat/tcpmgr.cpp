        ...
        QDataStream stream(&_buffer,QIODevice::ReadOnly);
          ...
          forever{//Qt里的一个无限循环
             ...
             if(!_b_recv_pending){
                 ...
                 //将buffer 中的前四个字节移除
                 _buffer = _buffer.mid(sizeof(quint16) * 2);//实际上是取出中间直到最后的数据再赋值
                 // 输出读取的数据
                 qDebug() << "Message ID:" << _message_id << ", Length:" << _message_len;

             }
              //buffer剩余length是否满足消息体长度，不满足则退出继续等待接受
             if(_buffer.size() < _message_len){
                  _b_recv_pending = true;
                  return;
             }
             _b_recv_pending = false;//接收完毕
             // 读取消息体
...
void TcpMgr::initHandlers()
{
    _handlers.insert(ID_CHAT_LOGIN_RSP,[this](ReqId id,int len,QByteArray data){
        ...
        // 将QByteArray转换为QJsonDocument
           ...
           auto user_info = std::make_shared<UserInfo>(uid, name, nick, icon, sex);//设置用户基本信息
    ...
    _handlers.insert(ID_SEARCH_USER_RSP,[this](ReqId id,int len,QByteArray data){ //处理搜索用户的请求
        Q_UNUSED(len);
        // 将QByteArray转换为QJsonDocument
               QJsonDocument jsonDoc = QJsonDocument::fromJson(data);
               // 检查转换是否成功
    ...
    _handlers.insert(ID_NOTIFY_AUTH_FRIEND_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        qDebug() << "handle id is " << id << " data is " << data;
        // 将QByteArray转换为QJsonDocument
        QJsonDocument jsonDoc = QJsonDocument::fromJson(data);//toJson,fromJson,字节流和jsonDocument转化
        ...
        if (!jsonObj.contains("error")) {
            int err = ErrorCodes::ERR_JSON;
            qDebug() << "Auth Failed, err is Json Parse Err" << err;
            return;
        }
        ...
        auto auth_info = std::make_shared<AuthInfo>(
                    from_uid, name,
                      icon, nick, sex);
...
void TcpMgr::slot_tcp_connect(ServerInfo si)
{
    qDebug()<< "receive tcp connect signal";
       ...
       _socket.connectToHost(si.Host, _port);//异步连接
}
...
void TcpMgr::slot_send_data(ReqId reqId, QByteArray data)
{
    uint16_t id = reqId;
       ...
       // 计算长度（使用网络字节序转换）
       quint16 len = static_cast<quint16>(data.length());//String类型处理中文会有问题

    ...
    : QListWidget(parent),_load_pending(false)
{
    ...
    //模拟从数据库或后端传来的数据进行列表加载
    addContactUserList();
    connect(this,&QListWidget::itemClicked,this,&ContactUserList::slot_item_clicked);
     //对端同意认证后通知的信号,收到服务器的回包，添加联系人
...
视口的位置可以任意移动,整个屏幕当作实际的物理坐标系,视口的坐标就是基于屏幕的坐标,
视口和窗口的长伸缩比比例和宽的伸缩比例可以是不一样的窗口(0,0,100,100),视口可以是(0,0,200,100)
 */
...
void ContactUserList::slot_auth_rsp(std::shared_ptr<AuthRsp>auth_rsp)
{
    qDebug() << "slot auth rsp called";
       ...
       auto *con_user_wid = new ConUserItem();
       con_user_wid->SetInfo(auth_rsp->_uid ,auth_rsp->_name, heads[head_i]);//设置对方的信息
       QListWidgetItem *item = new QListWidgetItem;
       //qDebug()<<"chat_user_wid sizeHint is " << chat_user_wid->sizeHint();
       item->setSizeHint(con_user_wid->sizeHint());

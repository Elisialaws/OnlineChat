...
bool MysqlDao::AddFriendApply(const int& from, const int& to)
{
	...
	try {
		// 准备SQL语句，use prepared statement for security and efficiency
		...
		//on duplicate key update,重复的主键或唯一的键发生冲突时
		pstmt->setInt(1, from); // from id
		pstmt->setInt(2, to);
		// 执行更新
		int rowAffected = pstmt->executeUpdate();//execute the update operation
		if (rowAffected < 0) {
			return false;
		}
		return true;
...
bool MysqlDao::AddFriend(const int& from, const int& to, std::string back_name) {
	...
	try {
		...
		//反过来的申请时from，验证时to
		pstmt->setInt(1, from); // set parameters, index,value
		pstmt->setInt(2, to);
...
std::shared_ptr<UserInfo> MysqlDao::GetUser(int uid)
{
	...
	Defer defer([this, &con]() {
		pool_->returnConnection(std::move(con));//确保连接能够返回到连接池
		});

...
CSession::CSession(boost::asio::io_context& io_context, CServer* server):
	...
	boost::uuids::uuid  a_uuid = boost::uuids::random_generator()();//标识会话
...
void CSession::Send(char* msg, short max_length, short msgid) {
	...
	if (send_que_size>0) { //队列>0，说明前面还有发的,多线程情况下
		return;
	}
	auto& msgnode = _send_que.front();
	boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len), 
...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec, std::size_t bytes_transfered) {
		try {
			...
			_recv_msg_node->_data[_recv_msg_node->_total_len] = '\0';//安全处理，最后一个字节为\0
...
void CSession::HandleWrite(const boost::system::error_code& error, std::shared_ptr<CSession> shared_self) {
	...
	try {
		if (!error) {
			...
			_send_que.pop();
            if (!_send_que.empty()) {
				auto& msgnode = _send_que.front();//取出数据异步写
				boost::asio::async_write(_socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
...
{
	::memset(_data, 0, MAX_LENGTH);//先清空之前的数据

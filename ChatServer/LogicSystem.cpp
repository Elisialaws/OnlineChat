...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short &msg_id, const string &msg_data) {
	...
	Json::Value  rtvalue;//返回的Json对象，包含各种数据
	...
	std::string token_key = USERTOKENPREFIX + uid_str;//token前缀+uid
	std::string token_value = "";
	bool success = RedisMgr::GetInstance()->Get(token_key, token_value);//取出服务器里的token
	if (!success) {
	...
	//为用户设置登录ip server的名字,因为用户可能在对端服务器
